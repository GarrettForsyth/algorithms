 ...
 *  the 5
 *  ///////////////////////////////////////////////////////////////////////////
 *  Modified to R = 26 for boggle letters (which are only capital alphabet letters
 *  Added a contains prefix function that returns immediately after a prefix is found
 *  Added a cache to prefix lookups
 ******************************************************************************/
...
public class TrieST26<Value> {
    ...
    private static final int MAX_PREFIX_CACHE_SIZE = 100000;
    ...
    public boolean containsPrefix(String prefix){
        /*
        String prevPrefix = prefix.substring(0, prefix.length()-1);
        if (prefixCache.containsKey(prevPrefix)){
            Node z = get(prefixCache.get(prevPrefix), prefix, prevPrefix.length()+1);
            if ( z != null) addToPrefixCache(prefix, z);
            return z != null;
        }
        */
    ...
    private void addToPrefixCache(String prefix, Node x){
        if (prefixCache.size() > MAX_PREFIX_CACHE_SIZE){
            prefixCache.remove(prefixCache);
        }
